...
class LitmusMLDPClassifier(IFreqaiModel):
    ...
    def train(
        ...
        # Keep legacy flow for smoke runs - avoid deprecated pipeline label transforms.
    ...
    def predict(
        ...
        dk.data_dictionary["predict_features"] = filtered_dataframe
        dk.do_predict = np.ones(len(filtered_dataframe), dtype=np.int_)
        ...
        # If training was skipped or failed, self.model may be None. Return safe empty preds.
        if self.model is None:
            logger.warning("No trained model available for prediction; returning zeros.")
            # build zeroed prediction DataFrame matching expected label columns
            num_rows = len(dk.data_dictionary["predict_features"])
            if not hasattr(dk, "label_list") or not dk.label_list:
                pred_df = DataFrame(index=range(num_rows))
            else:
                pred_df = DataFrame(
                    data=np.zeros((num_rows, len(dk.label_list))),
                    columns=dk.label_list,
                )

            # Ensure extra_returns_per_train entries align with prediction length
            extra = dk.data.get("extra_returns_per_train", {})
            for k, v in list(extra.items()):
                try:
                    if np.isscalar(v):
                        dk.data["extra_returns_per_train"][k] = np.full(num_rows, float(v))
                    elif hasattr(v, "__len__") and len(v) != num_rows:
                        # If it's a single-value list or different length, broadcast last element
                        try:
                            dk.data["extra_returns_per_train"][k] = np.full(num_rows, float(v[-1]))
                        except Exception:
                            dk.data["extra_returns_per_train"][k] = np.zeros(num_rows)
                except Exception:
                    dk.data["extra_returns_per_train"][k] = np.zeros(num_rows)

            do_predict = np.zeros(num_rows, dtype=np.int_)
            return (pred_df, do_predict)

        predictions = self.model.predict(dk.data_dictionary["predict_features"])
        pred_df = DataFrame(predictions, columns=dk.label_list)

        predictions_prob = self.model.predict_proba(dk.data_dictionary["predict_features"])
    ...
    def fit(self, data_dictionary: Dict, dk: FreqaiDataKitchen, **kwargs) -> Any:  # noqa: C901
        ...
        logger.info(f"Targets in train_labels: {list(data_dictionary['train_labels'].columns)}")
        for t in data_dictionary["train_labels"].columns:
            logger.info(f"Start training for target {t}")

            # Use correct train and test data assignments
            X_train = data_dictionary["train_features"]
            y_train = data_dictionary["train_labels"][t]

            time_scaler = MinMaxScaler(feature_range=(1, 10))
            time_weight_train = pd.DataFrame(
                time_scaler.fit_transform(
                    np.absolute(data_dictionary["train_weights"].reshape(-1, 1))),
                columns=["time_weight_train"], index=data_dictionary["train_labels"].index,
                dtype="float")

            # Robustly handle missing non-normalized data for weight calculation
            utility_train = data_dictionary.get("train_non_normalized", pd.DataFrame())
            weight_train_col = "!-trade_return"

            if not utility_train.empty and weight_train_col in utility_train.columns:
                weight_scaler = MinMaxScaler(feature_range=(1, 10))
                weight_train = pd.DataFrame(
                    weight_scaler.fit_transform(
                        np.absolute(utility_train.loc[:, [weight_train_col]])),
                    columns=["weight_train"], index=utility_train.index, dtype="float").fillna(value=1)
            else:
                logger.warning(f"Weighting column {weight_train_col} or train_non_normalized not found. Using uniform weights.")
                weight_train = pd.DataFrame(1.0, index=X_train.index, columns=["weight_train"])

            # Weight train can optionally include time weight
            # weight_train = weight_train + time_weight_train

            X_test = data_dictionary["test_features"]
            y_test = data_dictionary["test_labels"][t]
            time_weight_test = data_dictionary["test_weights"]
            
            # Robustly handle missing test_non_normalized
            utility_test = data_dictionary.get("test_non_normalized", pd.DataFrame())
            weight_test_col = "!-trade_return"
            
            if not utility_test.empty and weight_test_col in utility_test.columns and 'weight_scaler' in locals():
                weight_test = pd.DataFrame(
                    weight_scaler.transform(np.absolute(utility_test.loc[:, [weight_test_col]])),
                    columns=["weight_test"], index=utility_test.index, dtype="float").fillna(value=1)
            else:
                weight_test = pd.DataFrame(1.0, index=X_test.index, columns=["weight_test"])
            ...
            if self.shap_rfecv_enabled and self.shap_rfecv_rerun:
                ...
                pos_labels = [v for v in y_train.unique() if "win" in str(v)]
                if len(pos_labels) == 0:
                    logger.warning("No 'win' label found in y_train. Skipping model training for this fold.")
                    # mark that this target was skipped and continue to next target
                    continue
                ...
                trade_return_long = utility_train.get(
                    "!-trade_return_long", utility_train.get("!-trade_return", 0)
                )
                trade_return_short = utility_train.get(
                    "!-trade_return_short", utility_train.get("!-trade_return", 0)
                )
            ...
            # Support both string and numeric labels for win_labels
            if y_train.dtype == object or y_train.dtype == str:
                win_labels = [v for v in y_train.unique() if isinstance(v, str) and "win" in v]
            else:
                # Assume binary classification: 1.0 is positive label
                win_labels = [v for v in y_train.unique() if v == 1.0]
            if not win_labels:
                logger.warning("No 'win' (or 1.0) label found in y_train. Skipping model training for this fold.")
                return None
            pos_label = win_labels[0]
            shap_rfecv_params = self.freqai_info["feature_selection"]["shap_rfecv"]
            ...
            perf_scores = cross_validate(
                ...
                cv=cv, n_jobs=cv_params["cv_n_splits"])
        ...
        if not model_list:
            logger.warning("No models were successfully trained for any target. Returning None.")
            return None
