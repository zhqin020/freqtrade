...
class LitmusMLDPStrategy(IStrategy):
    ...
    def feature_engineering_standard(self, dataframe, **kwargs) -> pd.DataFrame:
        ...
        *Only functional with FreqAI enabled strategies*
        ...
        # Provide explicit max values to avoid division by zero when data contains
        # unexpected constant values for the period columns.
        ...
        for i, g in enumerate(primary_params["zigzag_min_growth"]):
            ...
            close_values = dataframe["close"].to_numpy(dtype=np.float64, copy=False)
            peaks = None

            def _simple_pivots(arr, up_thresh, down_thresh):
                # Simple pivot finder: marks +1 when price rises above last pivot by up_thresh,
                # -1 when it falls below last pivot by down_thresh. Not as robust as zigzag,
                # but sufficient for smoke tests.
                ...
                for i in range(1, len(arr)):
                    if last_pivot_price == 0:
                        ...
                        change = (arr[i] - last_pivot_price) / last_pivot_price
                    if direction <= 0 and change >= up_thresh:
                        res[i] = 1
                        last_pivot_price = arr[i]
                        direction = 1
                    elif direction >= 0 and change <= down_thresh:
                        res[i] = -1
                        last_pivot_price = arr[i]
                        direction = -1
                return res
            try:
                peaks = zigzag.peak_valley_pivots(close_values, min_growth, -min_growth)
            except Exception as exc:
                # Try alternate dtypes (float32) then python list before giving up
                logger.debug("zigzag initial call failed: %s; retrying with float32", exc)
                try:
                    peaks = zigzag.peak_valley_pivots(close_values.astype(np.float32), min_growth, -min_growth)
                except Exception as exc2:
                    logger.debug("zigzag float32 call failed: %s; retrying with list", exc2)
                    try:
                        peaks = zigzag.peak_valley_pivots(close_values.tolist(), float(min_growth), float(-min_growth))
                    except Exception as exc3:
                        logger.warning(
                            "zigzag labeling failed for %s (%s). Using simple fallback pivot finder.",
                            label,
                            exc3,
                        )
                        peaks = _simple_pivots(close_values, min_growth, -min_growth)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        smoothing_window = self.freqai_info["trigger_parameters"].get("smoothing_window", 30)
        if "threshold_meta_long_max_returns_&-meta_target" in dataframe.columns:
            dataframe["meta_enter_long_threshold"] = dataframe[
                "threshold_meta_long_max_returns_&-meta_target"].rolling(smoothing_window).mean()
        else:
            dataframe["meta_enter_long_threshold"] = 0

        if "threshold_meta_short_max_returns_&-meta_target" in dataframe.columns:
            ...
            dataframe["meta_enter_short_threshold"] = 0

        # Primary: Crosses over flag (guard if minima/maxima columns missing)
        minima_series = dataframe["minima_slow"] if "minima_slow" in dataframe.columns else pd.Series(0, index=dataframe.index)
        maxima_series = dataframe["maxima_slow"] if "maxima_slow" in dataframe.columns else pd.Series(0, index=dataframe.index)
        dataframe["primary_enter_long_trigger"] = qtpylib.crossed_above(
            minima_series, dataframe["primary_enter_long_threshold"])
        dataframe["primary_enter_short_trigger"] = qtpylib.crossed_above(
            maxima_series, dataframe["primary_enter_short_threshold"])
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        # Long Entry
        minima_series = df["minima_slow"] if "minima_slow" in df.columns else pd.Series(0, index=df.index)
        maxima_series = df["maxima_slow"] if "maxima_slow" in df.columns else pd.Series(0, index=df.index)
        a_win_long_series = df["a_win_long"] if "a_win_long" in df.columns else pd.Series(0, index=df.index)
        a_win_short_series = df["a_win_short"] if "a_win_short" in df.columns else pd.Series(0, index=df.index)
        conditions = [minima_series >= df["primary_enter_long_threshold"],
                  a_win_long_series >= df["meta_enter_long_threshold"]]
        ...
        # Short Entry
        conditions = [maxima_series >= df["primary_enter_short_threshold"],
                  a_win_short_series >= df["meta_enter_short_threshold"]]
        if conditions:
            df.loc[
    ...
    def adjust_trade_position(self, trade: Trade, current_time: datetime,
        ...
        a_win_long_series = dataframe["a_win_long"] if "a_win_long" in dataframe.columns else pd.Series(0, index=dataframe.index)
        a_win_short_series = dataframe["a_win_short"] if "a_win_short" in dataframe.columns else pd.Series(0, index=dataframe.index)

        conditions_long = [dataframe["primary_enter_long_trigger"],
                   a_win_long_series >= dataframe["meta_enter_long_threshold"],
                   current_profit < 0.0,
