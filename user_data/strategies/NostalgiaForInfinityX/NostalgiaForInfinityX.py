...
class NostalgiaForInfinityX(IStrategy):
    ...
    # Directional leverage settings used by leverage() callback.
    long_leverage = 3.0
    short_leverage = 2.0

    # Stepwise optimization controls.
    # both: run long+short together (recommended when tuning short params while keeping
    #       long params fixed and still trading).
    # short: short-only ablation mode (disables long entries).
    # long: long-only ablation mode (disables short entries).
    optimize_side = 'both'
    ...
    buy_params = {
        ...
        # Short-related hyperopt keys
        "short_1_cti_min": 0.69,
        "short_1_r14_min": -10.0,
        "short_1_rsi_min": 79.5,
        "short_2_cti_1h_min": 0.7,
        "short_2_rsi_1h_min": 81.5,
        "short_2_rsi_min": 69.6,
        "short_condition_1_enable": True,
        "short_condition_2_enable": False,
    }

    # Long-side parameters namespace (kept separate from short_params for stepwise tuning).
    long_params = buy_params

    sell_params = {
        "short_exit_cti_max": -0.67,
        "short_exit_profit_0": 0.026,
        "short_exit_profit_1": 0.02,
        "short_exit_profit_2": 0.033,
        "short_exit_r14_max": -62.4,
        "short_exit_rsi_max": 24.9,
        "short_exit_stoploss": -0.039,
    }

    # Short-side parameters (hyperopt/override friendly).
    short_params = {
        # Enable/Disable short entry conditions
        "short_condition_1_enable": True,
        "short_condition_2_enable": True,

        # Entry condition #1 (momentum short)
        "short_1_rsi_min": 66.0,
        "short_1_cti_min": 0.75,
        "short_1_r14_min": -18.0,

        # Entry condition #2 (overextended bounce rejection)
        "short_2_rsi_min": 70.0,
        "short_2_rsi_1h_min": 62.0,
        "short_2_cti_1h_min": 0.55,

        # Exit thresholds for short-side custom_exit
        "short_exit_profit_0": 0.01,
        "short_exit_profit_1": 0.02,
        "short_exit_profit_2": 0.03,
        "short_exit_stoploss": -0.05,
        "short_exit_rsi_max": 40.0,
        "short_exit_cti_max": -0.5,
        "short_exit_r14_max": -70.0,
    }

    # Hyperopt-ready short entry parameters (long side remains fixed).
    short_condition_1_enable = BooleanParameter(default=short_params["short_condition_1_enable"], space="buy", optimize=True)
    short_condition_2_enable = BooleanParameter(default=short_params["short_condition_2_enable"], space="buy", optimize=True)
    short_1_rsi_min = DecimalParameter(58.0, 82.0, decimals=1, default=short_params["short_1_rsi_min"], space="buy", optimize=True)
    short_1_cti_min = DecimalParameter(0.40, 0.95, decimals=2, default=short_params["short_1_cti_min"], space="buy", optimize=True)
    short_1_r14_min = DecimalParameter(-35.0, -5.0, decimals=1, default=short_params["short_1_r14_min"], space="buy", optimize=True)
    short_2_rsi_min = DecimalParameter(62.0, 88.0, decimals=1, default=short_params["short_2_rsi_min"], space="buy", optimize=True)
    short_2_rsi_1h_min = DecimalParameter(52.0, 84.0, decimals=1, default=short_params["short_2_rsi_1h_min"], space="buy", optimize=True)
    short_2_cti_1h_min = DecimalParameter(0.30, 0.95, decimals=2, default=short_params["short_2_cti_1h_min"], space="buy", optimize=True)

    # Hyperopt-ready short exit parameters.
    short_exit_profit_0 = DecimalParameter(0.005, 0.03, decimals=3, default=short_params["short_exit_profit_0"], space="sell", optimize=True)
    short_exit_profit_1 = DecimalParameter(0.01, 0.05, decimals=3, default=short_params["short_exit_profit_1"], space="sell", optimize=True)
    short_exit_profit_2 = DecimalParameter(0.015, 0.08, decimals=3, default=short_params["short_exit_profit_2"], space="sell", optimize=True)
    short_exit_stoploss = DecimalParameter(-0.12, -0.02, decimals=3, default=short_params["short_exit_stoploss"], space="sell", optimize=True)
    short_exit_rsi_max = DecimalParameter(20.0, 55.0, decimals=1, default=short_params["short_exit_rsi_max"], space="sell", optimize=True)
    short_exit_cti_max = DecimalParameter(-0.95, 0.10, decimals=2, default=short_params["short_exit_cti_max"], space="sell", optimize=True)
    short_exit_r14_max = DecimalParameter(-95.0, -35.0, decimals=1, default=short_params["short_exit_r14_max"], space="sell", optimize=True)
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # Short-side exits are separated from long logic to avoid mixing long-specific rules.
        if trade.is_short:
            if (
                current_profit > self.short_exit_profit_0.value
                and last_candle['rsi_14'] < self.short_exit_rsi_max.value
            ):
                return "sell_short_t_0"
            if (
                current_profit > self.short_exit_profit_1.value
                and last_candle['cti'] < self.short_exit_cti_max.value
            ):
                return "sell_short_t_1"
            if (
                current_profit > self.short_exit_profit_2.value
                and last_candle['r_14'] < self.short_exit_r14_max.value
            ):
                return "sell_short_t_2"
            if (current_profit > 0.0) and (last_candle['close'] > last_candle['ema_26']):
                return "sell_short_t_3"
            if current_profit < self.short_exit_stoploss.value:
                return "sell_short_sl_0"
            return None
    ...
    def leverage(self, pair: str, current_time: datetime, current_rate: float,
        ...
        if side == 'short':
            return min(self.short_leverage, max_leverage)
        return min(self.long_leverage, max_leverage)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        side_mode = str(self.optimize_side).lower()
        long_enabled = self.enable_long_entries and side_mode in ('both', 'long')
        short_enabled = self.enable_short_entries and side_mode in ('both', 'short')

        for index in self.buy_protection_params:
            item_buy_protection_list = [True]
            global_buy_protection_params = self.buy_protection_params[index]

            if long_enabled and self.long_params[f"buy_condition_{index}_enable"]:
                ...
                dataframe.loc[item_buy, 'enter_tag'] += f"long_{index} "
                conditions.append(item_buy)

        short_conditions = []

        # Momentum short in bearish context.
        if short_enabled and self.short_condition_1_enable.value:
            short_conditions.append(
                (dataframe['close'] < dataframe['ema_200']) &
                (dataframe['ema_26'] < dataframe['ema_12']) &
                (dataframe['rsi_14'] > self.short_1_rsi_min.value) &
                (dataframe['cti'] > self.short_1_cti_min.value) &
                (dataframe['r_14'] > self.short_1_r14_min.value) &
                (dataframe['volume'] > 0)
            )

        # Overextended bounce rejection below the long-term trend.
        if short_enabled and self.short_condition_2_enable.value:
            short_conditions.append(
                (dataframe['close'] < dataframe['ema_200']) &
                (dataframe['close'] > dataframe['bb20_2_mid']) &
                (dataframe['rsi_14'] > self.short_2_rsi_min.value) &
                (dataframe['rsi_14_1h'] > self.short_2_rsi_1h_min.value) &
                (dataframe['cti_1h'] > self.short_2_cti_1h_min.value) &
                (dataframe['volume'] > 0)
            )

        if short_conditions:
            short_signal = reduce(lambda x, y: x | y, short_conditions)
            dataframe.loc[short_signal, 'enter_tag'] += 'short_momentum '
        else:
            short_signal = False

        if conditions:
            long_signal = reduce(lambda x, y: x | y, conditions)
            dataframe.loc[:, 'buy'] = long_signal
            dataframe.loc[:, 'enter_long'] = long_signal.astype('int')
        else:
            dataframe.loc[:, 'buy'] = 0
            dataframe.loc[:, 'enter_long'] = 0

        if hasattr(short_signal, 'astype'):
            dataframe.loc[:, 'enter_short'] = short_signal.astype('int')
        else:
...
def pmax(df, period, multiplier, length, MAtype, src):
    ...
    pmx = np.where((pm_arr > 0.00), np.where((mavalue < pm_arr), 1.0,  0.0), np.nan)
...
'''
optimization:
(.venv) watson@u24:~/work/freqtrd$ source .venv/bin/activate && freqtrade hyperopt \
