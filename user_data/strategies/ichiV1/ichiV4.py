...
class ichiV4(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Debug alignment: with buy_trend_bullish_level=6, verify bullish EMA on 5m..4h.
        dataframe["debug_bullish_level"] = (
            ...
            & (dataframe["trend_close_1h"] > dataframe["trend_open_1h"])
            & (dataframe["trend_close_2h"] > dataframe["trend_open_2h"])
        ...
        # Debug alignment: shift(1/2/3) must all be below current value (V-shape allowed).
        dataframe["debug_fan_rising"] = (
            (dataframe["fan_magnitude"] > dataframe["fan_magnitude"].shift(1))
            & (dataframe["fan_magnitude"] > dataframe["fan_magnitude"].shift(2))
            & (dataframe["fan_magnitude"] > dataframe["fan_magnitude"].shift(3))
        ).astype(int)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # noqa: C901
        conditions = []
    ...
    def populate_short_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # noqa: C901
        conditions = []
        ...
        if conditions:
            dataframe.loc[reduce(lambda x, y: x & y, conditions), "short"] = 1
    ...
    def populate_cover_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        conditions.append(
            dataframe["fan_magnitude_gain"] > (2 - self.sell_fan_magnitude_gain.value)
        )
        conditions.append(dataframe["trend_close_5m"] > dataframe["trend_close_15m"])

        if conditions:
            dataframe.loc[reduce(lambda x, y: x & y, conditions), "cover"] = 1
