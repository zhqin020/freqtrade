...
class AlexStrategyTransfmr(IStrategy):
    """
    This is an example strategy that uses the Transformer model to predict the target score.
    ...
    def feature_engineering_expand_all(
        ...
        ) = ta.MACD(dataframe["close"], slowperiod=12, fastperiod=26)
    ...
    def set_freqai_targets(self, dataframe: DataFrame, metadata: Dict, **kwargs) -> DataFrame:
        new_cols = {}
        new_cols["ma"] = ta.SMA(dataframe, timeperiod=10)
        new_cols["roc"] = ta.ROC(dataframe, timeperiod=2)
        new_cols["macd"], new_cols["macdsignal"], new_cols["macdhist"] = ta.MACD(
            dataframe["close"], slowperiod=12, fastperiod=26
        )
        new_cols["momentum"] = ta.MOM(dataframe, timeperiod=4)
        new_cols["rsi"] = ta.RSI(dataframe, timeperiod=10)
        bollinger = ta.BBANDS(dataframe, timeperiod=20)
        new_cols["bb_upperband"] = bollinger["upperband"]
        new_cols["bb_middleband"] = bollinger["middleband"]
        new_cols["bb_lowerband"] = bollinger["lowerband"]
        new_cols["cci"] = ta.CCI(dataframe, timeperiod=20)
        new_cols["stoch"] = ta.STOCH(dataframe)["slowk"]
        new_cols["atr"] = ta.ATR(dataframe, timeperiod=14)
        new_cols["obv"] = ta.OBV(dataframe)
        # 一次性合并新列
        dataframe = pd.concat([dataframe, pd.DataFrame(new_cols, index=dataframe.index)], axis=1)
        ...
        # deviation of 1.
        dataframe["normalized_stoch"] = (
            dataframe["stoch"] - dataframe["stoch"].rolling(window=14).mean()
        ) / dataframe["stoch"].rolling(window=14).std()
        dataframe["normalized_atr"] = (
            dataframe["atr"] - dataframe["atr"].rolling(window=14).mean()
        ) / dataframe["atr"].rolling(window=14).std()
        dataframe["normalized_obv"] = (
            dataframe["obv"] - dataframe["obv"].rolling(window=14).mean()
        ) / dataframe["obv"].rolling(window=14).std()
        dataframe["normalized_ma"] = (
            dataframe["close"] - dataframe["close"].rolling(window=10).mean()
        ) / dataframe["close"].rolling(window=10).std()
        dataframe["normalized_macd"] = (
            dataframe["macd"] - dataframe["macd"].rolling(window=26).mean()
        ) / dataframe["macd"].rolling(window=26).std()
        dataframe["normalized_roc"] = (
            dataframe["roc"] - dataframe["roc"].rolling(window=2).mean()
        ) / dataframe["roc"].rolling(window=2).std()
        dataframe["normalized_momentum"] = (
            dataframe["momentum"] - dataframe["momentum"].rolling(window=4).mean()
        ) / dataframe["momentum"].rolling(window=4).std()
        dataframe["normalized_rsi"] = (
            dataframe["rsi"] - dataframe["rsi"].rolling(window=10).mean()
        ) / dataframe["rsi"].rolling(window=10).std()
        dataframe["normalized_bb_width"] = (
        ...
        strong_trend_threshold = (
            trend_strength.rolling(window=14).mean() + 1.5 * trend_strength.rolling(window=14).std()
        )
        ...
        dataframe["T"] = (
            dataframe["S"] * dataframe["R"] * dataframe["V"] * dataframe["R2"] * dataframe["V2"]
        )
